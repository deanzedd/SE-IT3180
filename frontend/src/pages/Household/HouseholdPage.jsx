...
const HouseholdPage = () => {
    ...
    const fetchHouseholds = async () => {
        try {
            setLoading(true);
            const response = await householdApi.getAll({ page, limit, search: searchTerm });
    ...
    const fetchDeletedHouseholds = async () => {
        try {
            const response = await householdApi.getAll({ isDeleted: true, page: deletedPage, search: deletedSearchTerm });
            const data = Array.isArray(response.data) ? response.data : response.data.data;
            setDeletedHouseholds(data || []);
    ...
    useEffect(() => {
        const delayDebounceFn = setTimeout(() => {
            if (showDeleted) {
                if (deletedPage === 1) fetchDeletedHouseholds();
            ...
            <Table 
                ...
                footerText={
                    <div className="flex justify-between items-center w-full">
                        <span>Hiển thị <span className="font-bold">{households.length}</span> / {totalHouseholds} kết quả</span>
                ...
                {showDeleted && (
                    <div className="mt-4 border-t pt-4">
                        <div className="flex justify-between items-center mb-3">
                            <h3 className="text-lg font-semibold text-gray-600">Danh sách căn hộ đã xóa</h3>
                            <div className="w-64">
